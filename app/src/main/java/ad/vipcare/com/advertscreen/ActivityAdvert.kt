...
//        advertPer!!.getwidthheight()
        ...
        advertPer!!.getRoot()
        // 申请磁盘权限
        advertPer!!.requestStoragePermiss()
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        // 监听读取返回的磁盘权限
        advertPer!!.onRequestSroragePermissions(requestCode, permissions  , grantResults)
    }
...
//        Log.i(TAG , "EventBus event.type：" + event.type )
        when(event.type){
            1 -> {
                ...
                if ("".equals(event.title)) {
                    var titleMsg: String = "正在拷贝文件"
                    showDownloadProgress(titleMsg)
                }else{
                    showDownloadProgress(event.title)
                }
                if(convenientBanner != null ) {
                    convenientBanner!!.stopTurning()
                }
     ...
     * 展示下载进取框
     */
    fun showDownloadProgress(title: String) {
        pd = DialogProgress(this@ActivityAdvert)
        pd!!.setDialogShow(title)
